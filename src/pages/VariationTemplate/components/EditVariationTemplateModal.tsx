...
interface EditVariationTemplateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export default function EditVariationTemplateModal({
  isOpen,
  onClose,
  onSuccess,
  template,
}: EditVariationTemplateModalProps) {
  ...
  const [updateVariationTemplate, { isLoading }] =
    useUpdateVariationTemplateMutation();

  // Reset form when template changes
  useEffect(() => {
    if (template && isOpen) {
      // Convert values to string array regardless of API format
      const valuesArray: string[] = typeof template.values === 'string'
        ? template.values.split(',').map(v => v.trim()).filter(v => v.length > 0)
        : template.values.map(v => v.value); // Extract 'value' property from VariationTemplateValue objects

      // Use setTimeout to ensure the form is properly reset after modal opens
      setTimeout(() => {
        reset({
          name: template.name,
  ...
  const onSubmit = async (data: CreateVariationTemplateFormData) => {
    ...
    try {
      const payload: UpdateVariationTemplatePayload = {
        id: template.id,
        body: {
          name: data.name.trim(),
      ...
      await updateVariationTemplate(payload).unwrap();
      toast.success("Variation template updated successfully");
      onSuccess();
      handleClose();
    } catch (error: any) {
      toast.error(error.data?.message || "Failed to update variation template");
    }
  };
  ...
  return (
    <Modal
      ...
      description="Update variation template with multiple values"
      className="max-w-2xl"
    >
